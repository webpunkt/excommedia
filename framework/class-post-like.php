	...
	class Businext_Post_Like {
		...
		public function process_simple_like() {
			...
			if ( ! wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
				exit( esc_html__( 'Not permitted', 'businext' ) );
			}
		...
		public function get_simple_likes_button( $post_id, $is_comment = null ) {
			...
			if ( $this->already_liked( $post_id, $is_comment ) ) {
				...
				$title = esc_html__( 'Unlike', 'businext' );
				$icon  = $icon_full;
			} else {
				$class = '';
				$title = esc_html__( 'Like', 'businext' );
				$icon  = $icon_empty;
			}
		...
		function get_liked_icon() {
			$icon = '<span class="sl-icon"><i class="ion-ios-heart"></i></span>';
		...
		function get_like_count( $like_count ) {
			...
			if ( $like_count > 1 ) {
				$number .= ' ' . esc_html__( 'likes', 'businext' );
			} else {
				$number .= ' ' . esc_html__( 'like', 'businext' );
			}
		...
		function show_user_likes( $user ) { ?>
			<table class="form-table">
				<tr>
					<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'businext' ); ?></label></th>
					<td>
						<?php
						...
						<?php else : ?>
							<p><?php esc_html_e( 'You do not like anything yet.', 'businext' ); ?></p>
							<?php
						endif;
						wp_reset_postdata();
						?>

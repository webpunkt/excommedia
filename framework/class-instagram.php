...
if ( ! class_exists( 'Businext_Instagram' ) ) {

	class Businext_Instagram {
		...
		public static function scrape_instagram( $username, $slice ) {
			...
			if ( false === $photos_array ) {
				...
				if ( is_wp_error( $remote ) ) {
					return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'businext' ) );
				}

				if ( 200 != wp_remote_retrieve_response_code( $remote ) ) {
					return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'businext' ) );
				}
				...
				if ( ! $insta_array ) {
					return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'businext' ) );
				}
				...
				if ( isset( $insta_array['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges'] ) ) {
					...
					return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'businext' ) );
				}

				if ( ! is_array( $images ) ) {
					return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'businext' ) );
				}
				...
				foreach ( $images as $image ) {
					...
					$caption = esc_html__( 'Instagram Image', 'businext' );
				...
				if ( empty( $photos_array ) ) {
					return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'businext' ) );
				}

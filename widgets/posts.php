...
if ( ! class_exists( 'TM_Posts_Widget' ) ) {
	class TM_Posts_Widget extends Businext_Widget {

		public function __construct() {

			$cat_options = array(
				'recent_posts' => esc_html__( 'Recent Posts', 'businext' ),
				'sticky_posts' => esc_html__( 'Sticky Posts', 'businext' ),
			);
			$categories  = get_categories( 'hide_empty=0' );
			if ( $categories ) {
				foreach ( $categories as $category ) {
					$cat_options[ $category->term_id ] = esc_html__( 'Category: ', 'businext' ) . $category->name;
				}
			}

			$this->widget_cssclass    = 'tm-posts-widget';
			$this->widget_description = esc_html__( 'Get list blog post.', 'businext' );
			$this->widget_id          = 'tm-posts-widget';
			$this->widget_name        = esc_html__( '[Businext] Posts', 'businext' );
			$this->settings           = array(
				'title'           => array(
					'type'  => 'text',
					'std'   => '',
				...
				'cat'             => array(
					'type'    => 'select',
					'std'     => 'recent_posts',
					'label'   => esc_html__( 'Category', 'businext' ),
					'options' => $cat_options,
				),
				'show_thumbnail'  => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__( 'Show Thumbnail', 'businext' ),
				),
				'show_categories' => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__( 'Show Categories', 'businext' ),
				),
				'show_date'       => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__( 'Show Date', 'businext' ),
				),
				'num'             => array(
					'type'  => 'number',
					'step'  => 1,
					...
					'max'   => 40,
					'std'   => 5,
					'label' => esc_html__( 'Number Posts', 'businext' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$businext_query = new WP_Query( $query_args );
								...
								<div class="post-widget-thumbnail">
									<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
										<?php if ( has_post_thumbnail() ) { ?>
											...
											Businext_Templates::image_placeholder( 80, 80 );
										}
										?>

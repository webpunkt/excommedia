...
if ( ! $api_key ) {
	$api_key = Businext::setting( 'google_api_key' );
}

wp_enqueue_script( 'gmap3' );
wp_enqueue_script( 'js-maps', BUSINEXT_PROTOCOL . '://maps.google.com/maps/api/js?key=' . $api_key . '&amp;language=en' );
                            ...
                            marker: {
                                values: [
									...
									?>
                                    {
                                        ...
                                        data: <?php printf( '%s', $_data );?>,
                                        options: {
											<?php if (! isset( $new_marker['icon'] ) || $new_marker['icon'] == '') { ?>
                                            icon: "<?php echo BUSINEXT_THEME_IMAGE_URI ?>/maps/map-marker.png",
											<?php } else {

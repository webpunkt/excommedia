 ...
 * Example
	array(
			'type' => 'gradient',
			'class' => '',
			'heading' => esc_html__('Gradient Type', 'businext'),
			'param_name' => 'bg_grad',
			'description' => esc_html__('At least two color points should be selected.', 'businext'),
			'dependency' => array('element' => 'enable_overlay', 'value' => array('enable')),
			'group' => $group_name,
		)
*/
if ( ! class_exists( 'Businext_VC_Gradient_Param' ) ) {

	class Businext_VC_Gradient_Param {
		...
		function gradient_picker( $settings, $value ) {
			...
			$output .= '<select id="grad_type' . $uni . '" class="grad_type" data-uniqid="' . $uni . '">
				<option value="vertical">' . esc_html__( 'Vertical', 'businext' ) . '</option>
				<option value="horizontal">' . esc_html__( 'Horizontal', 'businext' ) . '</option>
				<option value="custom">' . esc_html__( 'Custom', 'businext' ) . '</option>
			</select>
			...
			$output .= '<div class="wpb_element_label" style="margin-top: 10px;">' . esc_html__( 'Choose Colors', 'businext' ) . '</div>';
	...
	new Businext_VC_Gradient_Param();
}
